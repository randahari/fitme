...
## v1.1 — CANONICAL (CSF-01–16 approved; First Active V1 Reason Policy and Lifecycle-Aware B5 Eligibility closed; remaining items explicitly deferred/non-blocking)
...
> **Prepared by:** Lead Engineer / Repository Analyst / Canonical Decision Package Author, recording direction presented as approved by the Head of Product + AI Architect (CSF-01 through CSF-16), without reinterpretation, plus the deterministic engineering consequences that direction implies and the genuinely unresolved Product/Architecture items it does not itself close.
> **Repository baseline:** `main`, commit `30451b0f1fd790ff8edd6acd1b1eeee2b1f5259f` (`docs(g2): close pre-spec canonical decisions`).
...
> **Status of this version:** CANONICAL — CLOSED. CSF-01 through CSF-16 are approved Product + Architecture direction (unchanged from v1.0). **v1.1 records two further closures:** the **First Active V1 Reason Policy** — `REQUEST_SIGNIFICANTLY_IMPROVING_INFORMATION`, scoped exclusively to Habit-sourced `FOOD_LOGGING` degradation into the Habit Engine's `WEAKENING` lifecycle state (Chapter 26) — and the **Lifecycle-Aware B5 Eligibility Architecture Decision** — Habit-derived `WEAKENING` admission only, Pattern-derived `WEAKENING` explicitly excluded from v1 (Chapter 27). PD-1, PD-2, PD-4, AD-Detail-1, and AD-Detail-3 are reclassified DEFERRED / NON-BLOCKING (Chapters 11–12) — none prevents this Package from closing. `docs/specs/G2_SPEC_v1.0.md` is still not modified by this Package — see Chapter 24. A Canonical Synchronization Plan is recorded (Chapter 28) but not executed.
...
| Habit-sourced `FOOD_LOGGING` `WEAKENING` lifecycle | **CANONICALLY APPROVED — active v1 path (Chapter 26)**; B5-eligible per Chapter 27; engineering wiring (Initiative Engine's detector filter extension) remains, G2S revision |
...
Habit-sourced FOOD_LOGGING Observation
  → WEAKENING lifecycle (Habit Engine, structurally guaranteed prior confirmed-tier establishment)
...
## 27.1 Habit vs. Pattern `WEAKENING` — Repository-Verified Asymmetry

**Habit Engine** (`habitEngine.js:222-230`, `statusOf(conf, occ, daysSince, interval)`): the `weakening` branch (`late > 1.5`) is reached only *after* the function has already confirmed `occ ≥ OCC_CONFIRMED(5)` and `conf ≥ CONF_CONFIRMED(0.55)` earlier in the same function. **Habit-sourced `WEAKENING` status is therefore structurally impossible without prior confirmed-tier establishment** — a guarantee, not an inference.
...
**This Package is CANONICAL — CLOSED. CSF-01 through CSF-16 are approved. The First Active V1 Reason Policy (Chapter 26) and the Lifecycle-Aware B5 Eligibility Architecture Decision (Chapter 27) are approved and closed. PD-1, PD-2, PD-4, AD-Detail-1, and AD-Detail-3 are explicitly deferred/non-blocking, not silently invented or ignored. `docs/specs/G2_SPEC_v1.0.md` remains unmodified — see Chapter 24 for the revision it now requires. The Canonical Synchronization Plan (Chapter 28) is recorded but not executed.**
