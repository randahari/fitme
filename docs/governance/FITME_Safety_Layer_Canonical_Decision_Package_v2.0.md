...
## Repository Gaps
Covered by **GAP-01/GAP-02** (general disposition-policy gap) — no new gap unique to modification beyond what those two already capture, at the time this chapter was originally written. A further, narrower gap unique to modification's interaction with the tied-set Terminal Decision exception (Canonical Decision 7) surfaced later, during SL-001 SPEC authoring itself, and is separately tracked as **GAP-16** (Ch. 27) — RESOLVED by **RCD-15** (Ch. 28, v2.6).

## Completion Checklist
- **Already decided:** Contract shape and provenance-tracking (modification record, kind preservation). Its interaction with a tied-set Terminal Decision is now also decided (RCD-15, GAP-16).
- **Remains open:** The threshold/criteria for choosing modification over other dispositions; the bounded-modification content-generation algorithm (unaffected by RCD-15, which does not define it — see RCD-15's own Non-Goals).
- **Owner:** Product/AI Architect.
- **Blocks SPEC:** Yes, via GAP-01/GAP-02, for the still-open items above. GAP-16 no longer blocks (resolved).
...
### Newly Identified Gaps (v2.6 round — surfaced during SL-001 SPEC authoring itself, not inherited, not created by the original Ch. 01-26 analysis)

**GAP-16 — No Canonical Source Addressed the `MODIFIED`-Disposition/Tied-Set Interaction — RESOLVED**
- Gap Type: Repository / Architecture
- Repository Evidence: `[SL-001_SPEC_v1.0.md:429]` (authoring placeholder) and `[SL-001_SPEC_v1.0.md:483]`, tracked as **RG-3** within the working SL-001 SPEC's own local gap register — "no canonical source addresses the interaction between a `MODIFIED` Safety disposition and the narrow multi-option (tied-set) Terminal Decision exception... Not resolved by RCD-12, RCD-13, or RCD-14."
- Why this was a gap: Canonical Decision 7 (`[D2 Unit 08, D2-EF-05]`) established that a tied set is carried forward as one Terminal Decision with multiple user-selectable options, never split or collapsed; RCD-10/RCD-12/RCD-14 fixed the Safety Decision Matrix's evaluation unit as "the Terminal Decision under review," singular and undifferentiated. Neither source stated whether, or how, the Matrix's evaluation and the `MODIFIED` disposition's `modification` record apply when the Terminal Decision under review carries `options[]`. Every other disposition sidesteps the question by reforming the Terminal Decision's `kind` entirely (`[SL-001_SPEC_v1.0.md, Ch.26, CD-T006-06]`); `MODIFIED` alone preserves structure while altering content, making it the only disposition where the gap is live.
- Resolution: RCD-15 (Ch. 28) — the Head of Product + AI Architect approved decision-level (uniform) modification: the Safety Decision Matrix evaluates a tied-set Terminal Decision exactly as it evaluates any other Terminal Decision, with no per-option evaluation; `options[]` is preserved unmutated; and the resulting `modification` record is a Decision-Level Modification — a Safety-Layer-authored, whole-decision field, never scoped to an individual option.
- Blocks SPEC: **No** (resolved).

## Explicit Non-Interpretations
This chapter records resolutions approved by the Head of Product + AI Architect (Ch. 28); it does not itself resolve, reinterpret, improve, or replace any decision. GAP-06 and the inherited GAP-10 through GAP-13 remain open because no approved decision above addresses them — this chapter does not resolve them either. GAP-16 does not restate or alter GAP-01/GAP-02's own findings (Ch. 21's Modification Policy chapter already covers the general disposition-policy gap those cover); GAP-16 is scoped only to the tied-set interaction question RCD-15 resolves, and does not resolve the still-open bounded-modification content-generation algorithm (see RCD-15's own Non-Goals, Ch. 28).

## Repository Gaps
This chapter *is* the Repository Gaps register.

## Completion Checklist
- **Already decided:** GAP-01 through GAP-05, GAP-07, GAP-08, GAP-09, GAP-14, GAP-15, and GAP-16 are resolved, per the approved decisions recorded in Ch. 28; GAP-01 and GAP-02 are further, and now completely, resolved by RCD-12/RCD-14 and RCD-11/RCD-13 respectively.
...
**RCD-15 — RG-3 Resolution: Decision-Level Modification for Tied-Set Terminal Decisions — RESOLVED**
- Decision Statement: Approved. When Stage 9's Safety Decision Matrix returns disposition `MODIFIED` for a pre-review Terminal Decision assembled from the narrow permitted tied-set exception (Canonical Decision 7): (1) the Safety Decision Matrix evaluates the pre-review Terminal Decision exactly as it evaluates any other pre-review Terminal Decision — as one single, undifferentiated unit; no per-option evaluation, matching, or derivation is introduced; (2) `finalReview()` continues to return exactly one `SafetyReviewResult`, carrying exactly one `modifiedContent` object, per Decision Pass — no new field, method, parameter, or return shape is introduced to `SafetyIntegrationPort`; (3) the resulting Terminal Decision's `options[]` remains exactly as assembled by Winner Selection — identical count, order, membership, and `opportunityProvenance`; (4) the `modification` record is a **Decision-Level Modification** — a Safety-Layer-authored field of the Terminal Decision itself, existing at the same structural level as the Terminal Decision's other whole-decision fields (e.g., `rationale`, `safetyDisposition`), never at the level of, or scoped to, any individual member of `options[]`; it does not target, and shall not be construed as targeting, any one option more specifically than another. Every other Safety disposition continues to apply to a tied-set Terminal Decision exactly as already fixed by CD-T006-06; this decision does not alter their behavior.
- Canonical Rationale: Closes GAP-16 (Ch. 27). Two independently-approved canonical concepts — Canonical Decision 7 (tied-set structure preserved for user autonomy, `[TASK_006_SPEC_v1.0.md:933-937, 1695]`) and RCD-10/RCD-12/RCD-14 (Matrix evaluated against "the Terminal Decision under review," singular, `[SL-001_SPEC_v1.0.md:215, 217]`) — were never cross-examined against each other; `MODIFIED` is the one disposition where the collision becomes visible, since it alone preserves structure while altering content (`[SL-001_SPEC_v1.0.md, Ch.26, CD-T006-06]`). Of the architecturally valid resolutions considered (decision-level modification; option-specific bounded modification; categorical exclusion of `MODIFIED` for tied-sets), decision-level modification was selected because it requires no change to RCD-12/RCD-14's evaluation model, `SafetyIntegrationPort`'s contract, or RCD-13.D's invariants, and preserves every tied option intact, consistent with Canonical Decision 7's own user-autonomy rationale. This selection is a Product/Architecture choice among multiple canonically-valid architectures, not a conclusion uniquely derivable from prior text.
- Approval Evidence: Head of Product + AI Architect, Canonical Review — RG-3 Resolution Canonical Update.
- Documents Affected: `docs/specs/SL-001_SPEC_v1.0.md` (Ch.27, §31, Ch.29-30) — synchronized as part of this round (v2.6). `docs/specs/TASK_006_SPEC_v1.0.md` (§25, §25.10) — additive cross-reference only, no semantic change, synchronized as part of this round.
- Consequences: RG-3 is resolved; a tied-set Terminal Decision reaching `MODIFIED` now has a fully specified, testable outcome. The mixed-per-option-disposition question that would have arisen under the rejected option-specific alternative does not arise under this decision, since no per-option evaluation is introduced.
- Backward Compatibility: Additive only. No previously-shipped contract shape, field, or method (`SafetyIntegrationPort`, `SafetyReviewResult`, RCD-09 through RCD-14, Canonical Decision 7) is added, removed, or altered. The current repository implementation (`decisionFormation.js`) already carries `options[]` through the `MODIFIED` branch unmutated and attaches `modification.modifiedContent` as a decision-level sidecar field — this decision ratifies that existing behavior as intentional rather than requiring a code change.

## Explicit Non-Interpretations
This chapter records the Head of Product + AI Architect's approved decisions verbatim; it does not reinterpret, improve, or replace them, and it does not itself perform the repository updates RCD-06 requires — those remain outside this Decision Package's own authority (Ch. 01, Ch. 03). RCD-11 does not state, and this chapter does not invent, a migration mapping from the existing free-text `reason` field to the new closed `reasonCode` catalogue; RCD-13.E instead records, as an approved finding rather than an invented mapping, that no such migration table is required. RCD-14.C's Canonical Safety Rule Order is recorded exactly as approved — a fixed nine-value sequence, not inferred from `RiskType`'s enum declaration order and not derived by this chapter's own judgment. RCD-14.C's `Urgency` tie-break step is documented using `RCD-12.D`'s own already-approved per-value definitions to state the comparison direction, since the approved decision named `Urgency` as a criterion without restating a ranking string; this chapter treats that derivation as a direct, non-interpretive reading of already-approved text, not as an invented rule, and flags it here for transparency. RCD-15 does not define the bounded-modification content-generation algorithm, the Health/Safety Profile data source, or Expression's rendering behavior — each remains a separate, unresolved item outside this decision's scope, per RCD-15's own terms.

## Repository Gaps
N/A — this chapter records the resolution of Ch. 27's gaps via approved decisions; it does not introduce new gaps.

## Completion Checklist
- **Already decided:** RCD-01 through RCD-08 — approved (Canonical Review, Final Canonical Update); RCD-06 repository synchronization executed (Product Bible, AI Constitution, Coach Bible, Roadmap, Changelog, D1, D2, D3). RCD-09 through RCD-11 — approved (Canonical Review, Disposition Policy Canonical Update); repository synchronization executed (Roadmap, Changelog). RCD-12 through RCD-14 — approved by the Head of Product + AI Architect (Canonical Review, Ordered Safety Rule Framework Canonical Update for RCD-12/RCD-13, Canonical Safety Rule Evaluation Model Canonical Update for RCD-14, now fully documented including the Canonical Safety Rule Order); repository synchronization for RCD-12 through RCD-14 has been executed (SL-001 SPEC v1.0, per its Ch.01 Revision note). RCD-15 — approved by the Head of Product + AI Architect (Canonical Review, RG-3 Resolution Canonical Update); repository synchronization executed in this round (v2.6): SL-001 SPEC (Ch.27, §31, Ch.29-30) and TASK-006 SPEC (§25, §25.10, additive cross-reference only).
- **Remains open:** GAP-14 and GAP-15 are both fully resolved by RCD-14; GAP-16 is fully resolved by RCD-15. The free-text-`reason`-to-`reasonCode` migration-mapping item previously open under RCD-11 remains closed by RCD-13.E's finding that no such table is required. The bounded-modification content-generation algorithm and the Health/Safety Profile data source remain separately open, unaffected by RCD-15 (see RCD-15's own Non-Goals).
- **Owner:** Head of Product + AI Architect approved all fifteen decisions in full; repository synchronization for RCD-01 through RCD-15 is complete.
- **Blocks SPEC:** No — all fifteen RCDs fully resolved, no blocking gap remains open.
...
| RCD-15 (RG-3 resolution — Decision-Level Modification for tied-set) | RESOLVED | SL-001 SPEC (Ch.27, §31, Ch.29-30); TASK-006 SPEC (§25, §25.10, additive cross-reference only) |
...
This package itself, once accepted, becomes a candidate for citation by SL-001's future SPEC (RCD-01) — but it is not itself canonical (Ch. 01), and the physical repository updates listed above remain RCD-06's (for RCD-01 through RCD-08) or an equivalent follow-up action's (for RCD-09 through RCD-15) responsibility, not performed by this package.

## Explicit Non-Interpretations
This chapter does not sequence or prioritize the RCD-06 (or RCD-09–15) repository updates relative to each other — sequencing is a Product/AI Architecture planning decision. It does not restate the free-text-`reason`-to-`reasonCode` migration-mapping row previously carried here under RCD-11, since RCD-13.E's finding (no historical migration table required) closes it — that closure is recorded, not re-derived, in this chapter. GAP-15's row is removed from this table because GAP-15 is now fully resolved by RCD-14.C, per Ch. 27/28. RCD-15's row does not restate GAP-16's own evidence (Ch. 27) or RCD-15's own Non-Goals (Ch. 28) — both are recorded there, not re-derived here.

## Repository Gaps
None new.

## Completion Checklist
- **Already decided:** The mapping table above; every RCD-linked item is resolved. RCD-01 through RCD-11 are synchronized to their listed document(s) (RCD-01–08 via RCD-06; RCD-09–11 via Roadmap/Changelog, per repository commit). RCD-12 through RCD-14 are synchronized into SL-001 SPEC v1.0 (per that document's own Ch.01/header Revision note, "sync SLDP v2.5"). RCD-15 is synchronized into SL-001 SPEC and TASK-006 SPEC as part of this round (v2.6).
- **Remains open:** GAP-06, GAP-10 through GAP-13 (unaddressed, non-blocking, inherited) — informational only.
- **Owner:** Head of Product + AI Architect (all fifteen decisions approved in full; synchronization for RCD-01–15 complete).
- **Blocks SPEC:** No.
...
## Addendum (v2.6) — RCD-15

This addendum records a later event and does not alter the Final Verdict above, which concerned only the Skeleton §12 gate for SPEC *authoring* to proceed — a gate this package already found satisfied, and which SL-001 SPEC authoring has since acted on (SL-001 SPEC v1.0 exists and is synchronized through RCD-14, per its own Ch.01/header Revision note).

During SL-001 SPEC authoring, a further gap was identified that none of RCD-01 through RCD-14 addressed: **RG-3** (`[SL-001_SPEC_v1.0.md, Ch.27/§31]`), the interaction between the `MODIFIED` disposition and a tied-set Terminal Decision (Canonical Decision 7). This is recorded in this package as **GAP-16** (Ch. 27) and resolved by **RCD-15** (Ch. 28) — the Head of Product + AI Architect approved decision-level (uniform) modification: the Safety Decision Matrix evaluates a tied-set Terminal Decision as a single unit, `options[]` is preserved unmutated, and the `modification` record is a Decision-Level Modification applying to the Terminal Decision as a whole, never to an individual option. RCD-15 does not alter RCD-01 through RCD-14, Canonical Decision 7, or any other previously-resolved item. Repository synchronization for RCD-15 has been executed in this round: SL-001 SPEC (Ch.27, §31, Ch.29-30) and TASK-006 SPEC (§25, §25.10, additive cross-reference only, no semantic change).

**All fifteen Required Canonical Decisions (RCD-01 through RCD-15) are now resolved in full.** No unresolved canonical blocker remains for the Safety Layer; GAP-06 and the inherited GAP-10 through GAP-13 remain open, classified non-blocking, unaffected by this addendum.
