...
## Transitional Ratification

Approval of this version does not reopen specifications already approved under a prior version of this standard. TASK-004, TASK-005, and TASK-006 were approved under v1.0 and remain valid; they are not reopened by this revision. This ratification validates those prior approvals without retroactively changing the requirements that governed those specifications at the time they were approved.
...
-   **Lead Engineer responsibilities** — filling implementation detail, gathering repository evidence, defining test strategy, and reporting gaps or conflicts, strictly inside the boundaries Product and Architecture have already set.
...
The more-specific-source rule applies only where it does not conflict with the Source-of-Truth hierarchy documented in the Engineering Workflow (see Applicability). Where applying it would conflict with that hierarchy, Engineering does not apply it; the disagreement is recorded as a Canonical Conflict instead.
...
-   **Engineering Fill Instructions** — exactly what Engineering must produce for the concern.
...
Existing contracts are not modified by Engineering acting alone. A change to an existing contract requires explicit Product approval and explicit Architecture approval before Engineering may document or act on it. Engineering may document a proposed contract change within a specification for Product and Architecture review, but such a proposal carries no authority until explicitly approved by Head of Product + AI Architect.
...
During specification authoring, Engineering defines the Test Strategy (the required test cases, by name or scenario, per Per-Section Authoring Requirements); Engineering implements those tests only after the specification reaches READY, not during authoring.
...
These five categories are exhaustive; a specification does not create a sixth. "Follow-up" is not one of these primary unresolved-item categories; it is a reporting/closure classification that Head of Product + AI Architect may apply, at task closure, to an item already recorded under one of the five categories above — it always references an existing Repository Gap, Product Decision Pending, Architecture Decision Pending, Engineering Decision Pending, or Canonical Conflict, never stands on its own, and is not a status Engineering assigns during authoring (see Closure Record Requirements).
...
The prohibition on describing authoring history applies only to the normative body of the specification (every section other than Closure Record). The Closure Record is the sole section permitted to document implementation history, review history, corrections, approvals, lessons learned, and follow-up items — see Closure Record Requirements.
...
READY requires: a complete specification, a complete repository analysis, a complete test strategy, Engineering Self Review, Engineering Review, Product Review, and Architecture Review. READY does not require implementation, passing implementation tests, or regression execution — those belong to DONE.
...
DONE requires: implementation complete, all required tests passing, regression passing, documentation updated, and closure completed. The Product and Architecture sign-off at DONE (Final Product Verification and Final Architecture Verification) is a closure-time determination, distinct from the Product Review and Architecture Review performed at READY — see Engineering Self-Review Requirements.
...
This self-review (Engineering Self Review) is distinct from, and does not substitute for, the following: Engineering Review, the lifecycle gate preceding READY (see Applicability); Product Review and Architecture Review, the READY-stage reviews owned respectively by Head of Product and AI Architect (see Product Review Requirements and Architecture Review Requirements); and Final Product Verification and Final Architecture Verification, the closure-stage sign-offs performed at DONE (see DONE Requirements). Each is owned by the party named for it; Engineering does not perform or self-certify any review it does not own.
...
A task's closure record — dates, repository version, commit hash, implementation history, review history, corrections, approvals, documentation updates, final status, lessons learned, and follow-up work items — is written once, at actual task closure. It is never pre-filled speculatively during specification authoring, and it is left empty until closure actually occurs.

The Closure Record is the only section of a specification permitted to document authoring, implementation, and review history (see Forbidden Authoring Practices); every other section states what is true and what is required, not how it came to be so.
