...
## Safety Foundation Canonical Design

**Status:** 🟢 APPROVED AND CANONICAL
**Completion Date:** 2026-08-31
**Production Code Changes:** None

`docs/governance/FITME_Safety_Foundation_Canonical_Design_v1.0.md` freezes the Product/Architecture decisions underlying the Safety Foundation initiative: the three-Work-Item decomposition (SF-DECOMP-01 — Foundation A/User Safety Context, Foundation B/Minimum Action Identity, Foundation C/Canonical Safety Rule + Real Matcher, with A and B independent and C dependent on both being closed), Foundation A's canonical scope (explicit/literal user-reported restrictions only; Statement authority ≠ Interpretation authority), Foundation B's canonical scope (AD-SF-01 through AD-SF-04; a six-value closed initial activity vocabulary — RUNNING, WALKING, CYCLING, SWIMMING, STRENGTH_TRAINING, PADEL; full Action Model explicitly out of scope), and Foundation C's canonical scope (AD-SF-05; first-integration-point framing; SL-001 remains closed and untouched). Documentation-only; no implementation authorized or performed by this closure. Approved by Head of Product + AI Architect — see the document's own Chapter 11 (Status and Closure).
...
`docs/specs/USC_001_SPEC_v1.0.md` (Safety Foundation Work Item A) completed implementation, per the Engineering Workflow lifecycle. Implementation is complete: `js/coachDecisionSystem/safetyContextInterpreter.js` (new — a class-specific interpreter, architecturally identical in pattern to CSSC-001's/EUR-001's own interpreters, classifying explicit/literal user-reported Safety restrictions from Typed Memory only, never clinical inference), with `js/coachDecisionSystem/memoryLayer.js` extended (`assembleContext()` gains a new step populating `pipelineContext.userSafetyContext` / `pipelineContext.availability.userSafetyContext`), `js/app.js`/`index.html`/`sw.js` wiring, and the standard 17-file wiring-test version-literal update. New test file `tests/safetyContextInterpreter.test.js` and an extension to `tests/memoryLayer.test.js`. PD-USC-01 (temporal-qualifier preservation, `statedDurationText`, literal-only, no computed expiry) resolved before implementation. Closure Criteria (§21) met: the interpreter exists, passes its own classification/fail-closed/injection-containment tests, and populates a real `pipelineContext` field from synthetic Typed Memory fixtures, with zero dependency on Foundation B or C existing. Approved by Head of Product + AI Architect and closed — see `docs/specs/USC_001_SPEC_v1.0.md` for full details and evidence.
...
`docs/specs/MAI_001_SPEC_v1.0.md` (Safety Foundation Work Item B) completed implementation, per the Engineering Workflow lifecycle. Implementation is complete: `js/domain/activityIdentityVocabulary.js` (new, standalone module — a closed six-value activity vocabulary, RUNNING/WALKING/CYCLING/SWIMMING/STRENGTH_TRAINING/PADEL, independent of Safety per AD-SF-03 and independent of `domainTopicVocabulary.js` per AD-SF-04), and an optional, additive `Candidate.actionIdentity` field (absent/`null` for every existing Candidate, including the live FOOD_LOGGING Candidate, which is unaffected). New test files `tests/actionIdentityContract.test.js` and `tests/activityIdentityVocabulary.test.js`. AD-MAI-01 (the Stage 9 propagation boundary — `SINGLE_WINNER` does not propagate `actionIdentity`; Stage 9's candidate-null call path returns `[]` unconditionally) recorded during Implementation Review. No `index.html`/`sw.js`/wiring-test change — `actionIdentity` has no live Candidate producer at this baseline, an explicit, disclosed zero-live-producer scope (§15 of the SPEC), not a defect. Closure Criteria (§20) met: the vocabulary module exists, `Candidate.actionIdentity` is confirmed optional/additive by direct test against the real, unmodified Candidate-pool/winner-selection code path, and Stage 7 passthrough plus Stage 8 accessibility are proven via synthetic Candidate fixtures. Approved by Head of Product + AI Architect and closed — see `docs/specs/MAI_001_SPEC_v1.0.md` for full details and evidence.
...
`docs/specs/USP_001_SPEC_v1.0.md` (Safety Foundation prerequisite Work Item, downstream of A, upstream of C) completed implementation, per the Engineering Workflow lifecycle. Implementation is complete: `js/coachDecisionSystem/userSafetyProvenanceInterpreter.js` (new — a class-specific interpreter classifying the literal source a user attributes to a stated restriction, e.g. a medical source, from Typed Memory only), with `js/coachDecisionSystem/memoryLayer.js` extended (`assembleContext()` gains a new step populating `pipelineContext.userSafetyProvenance`), `js/app.js`/`index.html`/`sw.js` wiring, and the standard 17-file wiring-test version-literal update — unlike MAI-001, this Work Item's interpreter is wired into the real, executing `assembleContext()`, per the same convention USC-001 itself followed. New test file `tests/userSafetyProvenanceInterpreter.test.js` and an extension to `tests/memoryLayer.test.js`. PD-USP-02 (no title-recognition or proper-name-to-role conversion of any kind) resolved before implementation, confirmed by code review at commit time. Closure Criteria (§25) met: the interpreter exists, passes its own extraction/fail-closed/injection-containment tests, and populates a real `pipelineContext.userSafetyProvenance` field from synthetic Typed Memory fixtures, with explicit proof of non-interference with USC-001's own existing step, and zero dependency on Foundation C existing. Approved by Head of Product + AI Architect and closed — see `docs/specs/USP_001_SPEC_v1.0.md` for full details and evidence.
...
`docs/specs/CSR_001_SPEC_v1.0.md` (Safety Foundation C — the integration Work Item dependent on both USC-001 and MAI-001 being closed) completed implementation, per the Engineering Workflow lifecycle. Implementation is complete: `js/coachDecisionSystem/safetyLayer.js`'s `matchCanonicalSafetyRules()` — previously SL-001's own honest, disclosed empty stub (`return [];`) — now implements the repository's first real Canonical Safety Rule, `matchRunningMedicalRestrictionRule`: an explicit user-reported medical restriction against RUNNING (USC-001's `userSafetyContext`) joined via `sourceMemoryId` to a matching medical-source attribution (USP-001's `userSafetyProvenance`), matched against a Candidate whose `actionIdentity.activity === 'RUNNING'` (MAI-001's identity), disqualifying it at Stage 8. PD-FC-07 (a closed, precomputed Hebrew accepted-form set — token / ה+token / ו+token / וה+token, never `\b`-based) and PD-FC-08 (the alphanumeric token-boundary correction — letters and numbers are both token constituents) resolved before implementation. Stage 8 `disqualify()` and Stage 9 `finalReview()` signatures are unchanged; Stage 9's candidate-null call path continues to return `[]` unconditionally per AD-MAI-01. New test file `tests/canonicalSafetyRule.test.js` (30 tests) and an extension to `tests/safetyLayer.test.js` (+10 `CSR1-*` tests), both exercising CSR-001 exclusively through the existing public canonical Safety API. Full repository regression at this closure, per the implementation commit's own report: **2330/2330 passing**. SL-001 remains closed and untouched — Foundation C extends SL-001's own already-approved stub with real logic; it does not reopen, redesign, or amend SL-001's closed enums or ordered Safety Decision Matrix. **Reachability note:** the rule is implemented, executable, and integrated into the existing Stage 8/9 Safety evaluation path, proven end-to-end against synthetic Candidate/`pipelineContext` fixtures. No live Candidate producer in the current repository (`recommendationEngine.js`/`initiativeEngine.js`) attaches `actionIdentity` to a real Candidate, so the rule does not yet act on any live, production-generated Candidate — a matter of upstream production reachability (the absence of a live RUNNING-activity Candidate producer), not an incomplete or infrastructure-only CSR-001 implementation. Approved by Head of Product + AI Architect and closed — see `docs/specs/CSR_001_SPEC_v1.0.md` for full details and evidence.
...
Safety Foundation Canonical Design — the Product/Architecture decision package freezing the Foundation A/B/C decomposition (SF-DECOMP-01) — is approved and Canonical, per `docs/governance/FITME_Safety_Foundation_Canonical_Design_v1.0.md`; documentation-only, no production code changes.
USC-001 — User Safety Context V1 (Safety Foundation A) is approved, implemented and closed, per `docs/specs/USC_001_SPEC_v1.0.md` — a class-specific interpreter populating `pipelineContext.userSafetyContext` from explicit, literal user-reported Safety restrictions only.
MAI-001 — Minimum Action Identity V1 (Safety Foundation B) is approved, implemented and closed, per `docs/specs/MAI_001_SPEC_v1.0.md` — a closed six-value activity vocabulary and an optional, additive `Candidate.actionIdentity` field, with no live Candidate producer at this baseline (disclosed, not a defect).
USP-001 — User Safety Provenance V1 (Safety Foundation prerequisite, downstream of A, upstream of C) is approved, implemented and closed, per `docs/specs/USP_001_SPEC_v1.0.md` — a class-specific interpreter populating `pipelineContext.userSafetyProvenance`.
CSR-001 — Canonical Safety Rule V1 + Real Matcher (Safety Foundation C) is approved, implemented and closed, per `docs/specs/CSR_001_SPEC_v1.0.md` — the repository's first real Canonical Safety Rule, joining USC-001's and USP-001's Pipeline Context with MAI-001's Candidate identity at Stage 8; implemented, executable, and integrated into the Safety evaluation path, but not yet reachable end-to-end against a live production Candidate for lack of a live `actionIdentity` producer — an upstream production-reachability gap, not an incomplete implementation. This closes the Safety Foundation initiative's own first real vertical (A, B, and C all closed).
TRR-001 — Training Readiness & Recovery V1 is approved, implemented and IMPLEMENTED / VERIFIED / CLOSED, per `docs/specs/TRR_001_SPEC_v1.0.md` — the first live vertical instantiating the Training Readiness & Recovery V1 Canonical Decision Package and the Context-Aware AI Reasoning Foundation, adding the 8th Product Reason (`ADAPT_TO_CURRENT_STATE`), a second per-activity Canonical Safety Rule (WALKING), and a generic Unresolved Activity Safety Coverage Rule producing `DEFERRED` for genuinely unresolved Safety relevance. The native-speaker Hebrew linguistic review of `WALKING_TEXT_ACCEPTED_FORMS` was completed and APPROVED by Product/Canonical review before this closure (deterministic test coverage recorded in addition, per `docs/specs/TRR_001_SPEC_v1.0.md` §46). No item remains open. TRR-specific regression: 433/433 passing.
Stage-9 Winning-Candidate Safety Input — a narrow Product/Architecture canonical correction discovered during TRR-001's own pre-commit verification and IMPLEMENTED / VERIFIED / CLOSED, per `docs/governance/FITME_Stage9_Winning_Candidate_Safety_Input_Canonical_Decision_v1.0.md` — Stage 9's `finalReview()` now receives the real winning Candidate for a `SINGLE_WINNER` Terminal Decision, restoring live reachability of the full Safety Decision Matrix; `TIED_SET` and Stage 8 unchanged. Critical production evidence: a CYCLING Candidate against an unresolved Safety restriction now resolves `DEFERRED` → `SILENCE`, not delivered. Full repository regression: 2478/2478 passing (2465 pre-correction baseline, net +13).
DUC-001 — Direct User Coach Admission V1 is approved, implemented and IMPLEMENTED / VERIFIED / CLOSED, per `docs/specs/DUC_001_SPEC_v1.0.md` — the first Work Item admitting a real, direct user message into the governed Coach Decision System (`USER_MESSAGE_SUBMITTED` / `DIRECT_TURN_PASS`), reusing Stage 4–10 (Evidence, Eligibility, Reasoning, Safety, Decision Formation, Expression) unmodified. New: bounded, domain-agnostic Turn Understanding and a Conversational Need Creator (the fifth Stage-3 contributor); a new `TerminalDecision.kind = 'UNSUPPORTED'` for a recognized-but-unsupported request, rendered through its own governed Expression path; a shared, domain-agnostic Coach Conversation Surface UI. TRR (Training Readiness & Recovery) is the first supported professional conversational capability, reached via the existing, unmodified `ADAPT_TO_CURRENT_STATE` reasoning gate. A post-implementation Product/Architecture review found and corrected a genuine turn-serving defect — an unrelated proactive Opportunity could displace the Candidate answering the originating turn — via a narrow, domain-agnostic admission filter (scoped to `turnId`/`safetyHighRiskBypass`, never `sourceCategory`) and a narrow `DIRECT_USER_REQUEST × ADAPT_TO_CURRENT_STATE → hierarchyTier 5` Source×Reason override, replacing a removed global source-only tier mapping. Safety remains fully authoritative and unconditional; `APP_READY` and proactive TRR are unchanged; no raw Claude → DOM path exists; no automatic Memory persistence; Preference V1 remains paused. No item remains open. Implementation commit `4d4e4bf48b75ca09b7d8b31ba9c56d5c09d6dcff`. Full repository regression: 2559/2559 passing.

Current Work Item: None. Phase C (C1–C4), the D-series (D1–D3), TASK-004 (Recommendation Engine), TASK-005 (Initiative Engine), TASK-006 (Decision Engine), SL-001 (Safety Layer), TASK-007 (UX System), TASK-008 (Design System), Expression (D3 §17's sixth and final Coach Decision System collaborator), G-2 (Live Stage 3/4 Opportunity Recognition), RGEF (Relationship-Guided Engagement Foundation), ESAF-001 (Explicit User Statement Arrival Freshness), CSSC-001 (Current State / Situational Context V1), EUR-001 (Explicit User Request V1), LCSC-001 (Legacy Coach Safety Containment), the Safety Foundation Canonical Design, USC-001 (User Safety Context V1), MAI-001 (Minimum Action Identity V1), USP-001 (User Safety Provenance V1), CSR-001 (Canonical Safety Rule V1 + Real Matcher), the Stage-9 Winning-Candidate Safety Input correction, TRR-001 (Training Readiness & Recovery V1), and DUC-001 (Direct User Coach Admission V1) are complete and closed. (Note: this list pre-existingly omits USM-001 — Authoritative User Understanding Foundation, First Vertical, which has its own full section above; that omission predates this closure and is left uncorrected here, as it is unrelated to this Work Item's own scope.)
Next Work Item: Not yet designated. The next Product/Architecture activity will be determined separately through Canonical Work Item Selection — this Roadmap does not name or assume a successor task.
