...
- 🟢 USM-001 — Authoritative User Understanding Foundation, First Vertical — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/USM_001_SPEC_v1.0.md`; the first Work Item proving Model B — several producer stores/derived engines → one authoritative semantic projection assembled by Memory Layer — with the smallest possible new surface: an existing, manually user-stated Typed Memory fact/preference becomes an authoritative Coaching Decision Input reaching Coach Prompt Composition, observably personalizing its content, fully correctable and forgettable. **New:** `js/userStatedMemoryPrompt.js` (dedicated bounded projector, independent `MAX_FACTS`/`MAX_CHARS`). **Modified, additively only:** `js/stateAccess.js` (new `userStatedMemory` read op + new `memoryLayer`/`USER_STATED_MEMORY_READ` capability-holder identity — not an Engine, not an alias/widening of `coachDecisionSystem`/`DECISION_PASS`; consent-gated fail-closed to `[]` before any fetch; filters to `type ∈ {fact,preference}` × `source:'user_stated'` × `status:'active'`; deterministic ordering), `js/coachDecisionSystem/memoryLayer.js` (new, independent `assembleUserStatedMemoryFragment()`, structurally separate from `assembleContext()`, which is byte-identical before/after), `js/coach/coachPromptComposer.js` (`buildSystemPrompt()` gains one new, structurally distinct fragment step between the legacy `coachMemoryFragment()` and B5's derived fragment — no Domain/Topic/Opportunity/Trust/Goal classification performed), `js/app.js` (one new injected StateAccess dependency reusing `js/memory.js`'s existing `list()`; no new Firestore query). An Engineering Readiness Review found and resolved a genuine script-load-order conflict before implementation (relocating `js/coachDecisionSystem/expressionRenderingContext.js` + `memoryLayer.js` together, verified safe, ahead of `coachPromptComposer.js`, in place of an initially-proposed lazy-resolution workaround); during implementation this relocation was found to violate one existing wiring test's own broader-than-necessary invariant, and two further tests asserted the exact prior literal source line of `buildSystemPrompt()`'s final return statement — all three mechanically corrected, every other assertion preserved. **Production-backed verified** end-to-end using the real, unmodified-elsewhere `js/stateAccess.js`/`memoryLayer.js`/`userStatedMemoryPrompt.js`/`coachPromptComposer.js` (only the Firestore-backed `js/memory.js` CRUD boundary simulated, via `js/memory.js`'s own real `makeMemory()`/`validateMemory()`): consent false → absent, fetch never attempted; consent true → present under its own header; edit → corrected value only; delete → gone; consent revoke → no Typed Memory content, no reload; legacy fragment coexists without interference; mid-fetch staleness degrades honestly; cross-user isolation confirmed. 5 test files added/extended; **full suite 1997/1997 passing** (1946 pre-USM-001 baseline, net +51). `APP_VERSION`/service-worker `VERSION` advanced from `2.42.0`/`v2.42.0` to `2.43.0`/`v2.43.0` — genuinely new, user-visible Coach behavior (personalized prompt content) ships. Open/deferred: `type:'preference'` has no live producer (RG-1), historical supersession, Chat/Conversation/Voice, C4 producer wiring, explicit-request scope/duration, temporal/future-event memory, Goal+Why, Relationship Maturity, Trust (Affirmative Trust V1 remains paused, unmodified, to be reviewed after this foundation), Habit/Pattern mirroring, medical/safety semantic classification.)
- 🟢 ESAF-001 — Explicit User Statement Arrival Freshness — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/ESAF_001_SPEC_v1.0.md`; connects USM-001's existing manual Typed Memory write path to the Composite Engine's own pre-existing, previously-dormant D2-EF-07 pre-dispatch supersession check (`internalPipelineOrchestrator.js`), proving the narrow correctness guarantee that new authoritative user information can invalidate a Decision assembled before it arrived — with zero semantic interpretation, classification, Domain/Topic, Evidence Tier, Trust, or Relationship Maturity involvement. **Modified, additively only:** `js/memory.js` (new `esafQualifies()`/`esafSignalArrival()` internal helpers reusing the exact USM-001 read-path-visibility filter — `type ∈ {fact,preference}` × `source:'user_stated'` × `status:'active'`; five existing D6 write-action handlers — create, edit, "לא נכון" reject, delete, and the memory-consent checkbox — each call the existing, unmodified `MemoryLayer.recordExplicitUserStatementArrival({userId})` after their own already-existing write verifiably succeeds, content-blind, never before). A SPEC Review found and corrected two real, evidence-backed defects before implementation: **(1)** `saveProfile()` never throws on persistence failure (it returns `{status:'FAILED'}`), so the consent call site gates on the returned `status==='SUCCESS'`, not on absence-of-throw; **(2)** consent *grant* (`false→true`) was found to require the identical treatment as Create — both change the exact same USM-001 read-path-visible set in the same direction — so both consent directions signal, not revoke alone. A full traced production-call-graph review (`registerCoachDecisionSystem.js` → `engineRegistry.js` → `app.js`) confirmed this mechanism's only live trigger (`APP_READY`) is background/non-blocking with no synchronous user-awaited consumer, so no Product "silent non-response" risk exists today; no reassembly/retry is added — a superseded background pass is silently withheld, exactly as D2-EF-07 already specified, and a later `APP_READY` pass naturally reassembles fresh. Two pre-existing, unrelated limitations of the already-approved arrival mechanism (per-tab-only in-memory arrival state; a harmless stale dictionary key surviving a same-tab user switch) were verified, reported, and explicitly left unfixed per the SPEC's own instruction not to add cross-tab synchronization. New test file `tests/esaf001.test.js` (16 tests: qualifying-filter, content-blind signal, per-user scoping, and a production-backed acceptance vertical against the real, unmodified `memoryLayer.js`/`internalPipelineOrchestrator.js` proving the actual T1-assembly → T2-arrival → `SUPERSEDED` → no-Expression-dispatch chain); **full repository regression: 2013/2013 passing, 0 failing** (1997 pre-ESAF-001 baseline, net +16). `APP_VERSION`/service-worker `VERSION` inspected at this closure and left unchanged (`2.43.0`/`v2.43.0`), matching G-2's own precedent: this closure makes an existing, previously-dormant correctness mechanism reachable for the first time, but produces no new user-visible Coach behavior on its own (the Composite Engine's only live trigger remains background/autonomous with no live Opportunity source). Open/deferred, explicitly not resolved by this closure: js/memory.js's D6 UI click handlers remain outside this repository's established Node-testing boundary (pre-existing, unchanged by this Work Item); Semantic User Understanding (classification, Domain/Topic, semantic projection); the Explicit Request immediate-suppression Product rule (recorded, not implemented); Conversation/Voice producers (architecturally compatible, not built); Decision Evidence, Trust, and Relationship Maturity (all untouched). Approved by Head of Product + AI Architect and closed — see `docs/specs/ESAF_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 CSSC-001 — Current State / Situational Context V1 — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/CSSC_001_SPEC_v1.0.md`; the first real, end-to-end Semantic User Understanding vertical — AUTHORITATIVE USER STATEMENT → DERIVED SEMANTIC INTERPRETATION → CURRENT STATE / SITUATIONAL CONTEXT → MEMORY LAYER → the existing, live `FOOD_LOGGING`/`WEAKENING` Contextual Meaning consumer, as truthful non-causal background only. Statement authority ≠ interpretation authority throughout: the classification is Tier-5/derived, never persisted, never promoted to authoritative memory. **New:** `js/coachDecisionSystem/situationalContextInterpreter.js` — owns the entire classification act (closed two-token vocabulary `CLASSIFIED_CURRENT_STATE`/`INELIGIBLE_OR_NOT_CLASSIFIED`, deterministic id-keyed batching over the *complete* eligible record set with no total-count truncation, per-batch timeout, no retry, strict id-matching output validation with fail-closed handling for missing/duplicate/unknown/malformed results, per-id-delimited prompt-injection containment, unconditional prompt-level abstention on health/safety-adjacent content) — auth reused via the existing `callClaude` closure (`js/app.js`), never a Firebase Auth object through Decision identity. **Modified, additively only:** `js/coachDecisionSystem/memoryLayer.js` (`assembleContext()` gains one new step: a cost-only mechanical pre-check for a live `HABIT`/`FOOD_LOGGING`/`WEAKENING` signal before any classification is attempted; reuses USM-001's existing `memoryLayer`/`USER_STATED_MEMORY_READ` StateAccess identity unchanged, never widening `coachDecisionSystem`/`DECISION_PASS`'s own grant; new `situationalContext.items[]` Pipeline Context field), `js/coachDecisionSystem/contextualMeaningPolicy.js` (its one V1 rule gains `contextConsulted.situationalContext`/`basis.situationalContextBackground` — non-causal only, never read by `deriveValidReasonCategory`, confirmed unreachable by direct inspection), `js/app.js`/`index.html`/`sw.js` (wiring). A Final SPEC Correction resolved two Engineering Readiness findings before implementation: eligible-record completeness must never be bounded by a total-count cap (transport batching replaces the earlier, rejected "top-6" truncation), and the Safety contract is an explicit closed-vocabulary eligibility boundary, not a blast-radius argument. **Engineering discovery during implementation (documentation correction, not an architecture change):** this V1's Habit fixture does not resolve to Silence in the current repository — RGEF (closed, pre-dating and independent of this Work Item) already admits it to a live `INITIATIVE` Terminal Decision; proven, not merely asserted, that Situational Context changes neither that outcome nor its content (`kind`/`rationale`/`decisionPassTrace` byte-identical with and without it) and cannot structurally reach Expression's rendering payload at all (`buildExpressionRenderingContext()`'s pre-existing, unmodified relationshipMaturity-stage-only pass-through). New test files `tests/situationalContextInterpreter.test.js` (26 tests) and `tests/cssc001ProductionBackedAcceptance.test.js` (5 tests); `tests/memoryLayer.test.js` (+10) and `tests/contextualMeaningPolicy.test.js` (+4) extended; **full repository regression: 2058/2058 passing, 0 failing** (2013 pre-CSSC-001 baseline, net +45). `APP_VERSION`/service-worker `VERSION` inspected at this closure and left unchanged (`2.43.0`/`v2.43.0`): this Work Item adds no new occurrence, timing, or content to any dispatch — the live `INITIATIVE` path it happens to touch already existed via RGEF, proven unaffected. Open/deferred, explicitly not resolved by this closure: any Domain/Topic assignment; Preference/Life-Event/Explicit-Request/Intervention-Feedback semantic classes; Conversation/Voice producers (architecturally compatible via the same producer-neutral raw-statement contract, not built); a non-authoritative cost-only classification cache (not needed at pilot scale); Trust, Relationship Maturity, and Goal architecture (all untouched). Approved by Head of Product + AI Architect and closed — see `docs/specs/CSSC_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 EUR-001 — Explicit User Request V1 — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/EUR_001_SPEC_v1.0.md`; the second real, end-to-end Semantic User Understanding vertical — AUTHORITATIVE USER STATEMENT → EXPLICIT-REQUEST CLASSIFICATION → CONTROL-INTENT INTERPRETATION → LITERAL-SCOPE RESOLUTION → DIRECT-USER AUTHORITY → Initiative Engine Stage 6, the first real, behavior-changing consumer of this foundation. Three independent, never-collapsed semantic dimensions (`requestClassification`/`controlIntent`/`scopeStatus`+`domain`/`topic`) feed one single conjunctive actionable-control gate: only `CLASSIFIED_EXPLICIT_REQUEST` + `SUPPRESS_ORDINARY_INITIATIVE` + `RESOLVED` + a valid closed Domain/Topic pair ever produces a control — a positive/supportive request (e.g. "Please remind me to log my food.") is recognized but structurally inert; a suppressive-but-unresolved-scope request (e.g. "Don't suggest running.") is recognized but never fake-mapped onto `WORKOUT`/`WORKOUT_FREQUENCY`. **New:** `js/coachDecisionSystem/explicitRequestInterpreter.js` — a separate, class-specific interpreter reusing `situationalContextInterpreter.js`'s proven architectural skeleton by pattern (deterministic id-keyed batching over the *complete* eligible set, per-batch timeout, no retry, strict fail-closed output validation including gating-dimension consistency, per-id prompt-injection containment, no numeric confidence) but never merged with it — CSSC and EUR remain two separate, non-merged interpreters, an accepted V1 cost (independent batches/model calls over potentially the same source set, and a potential duplicate Typed Memory read within one Decision Pass, both explicitly documented rather than optimized away, since semantic authority separation takes precedence for V1). Its own closed `(domain, topic)` pairing table is independently authored, informed by — but never calling into — `derivedIntelligenceConsumer.js`'s separate, unmodified B5 mapping. Auth reused via the existing `callClaude` closure (`js/app.js`), never a Firebase Auth object or token through Decision identity — an Engineering Readiness Review correctly caught and closed a `getAuthToken` seam inaccuracy before implementation began. **Modified, additively only:** `js/coachDecisionSystem/memoryLayer.js` (`assembleContext()` gains one new step — deliberately with *no* pre-check gate, unlike `situationalContext`'s `WEAKENING`-signal gate, since Explicit Request's real consumer is broader than one Contextual Meaning rule; reuses USM-001's existing `memoryLayer`/`USER_STATED_MEMORY_READ` StateAccess identity unchanged; new `explicitRequestControls.items[]` Pipeline Context field containing only already-actionable controls, never raw/intermediate classifier output), `js/coachDecisionSystem/initiativeEngine.js` (`generate()` gains one new, additive, independent OR-branch beside the existing RGEF WP7 `domainTopicRecentlyUnwelcome()` check — direct-user authority requires no repeated-dismissal threshold, no Trust, no Relationship Maturity, unlike RGEF's own inferred-reluctance mechanism, which remains fully untouched and structurally separate; no new dependency on `feedbackDomain.js`), `js/app.js`/`index.html`/`sw.js` (wiring). Two SPEC Review rounds resolved two real, evidence-backed defects before implementation: **(1)** the original two-dimension contract conflated "is this a request" with "is this suppressive" — corrected to three independent dimensions with request classification and control intent kept structurally distinct, since a clearly-recognized Explicit Request (e.g. "Help me stay consistent with food logging.") must never be force-mapped to suppression; **(2)** the proposed `configure({getAuthToken})` auth seam matched neither this repository's real, shipped convention nor `situationalContextInterpreter.js`'s own actual implementation — corrected to the real `configure({callClaude})` closure-injection pattern before any code was written against it. **Production-backed verified** using the real, unmodified `memoryLayer.js`, `contextualMeaningPolicy.js`, `initiativeEngine.js`, `eligibilityEvaluator.js`, and `internalPipelineOrchestrator.js`, with only `ExplicitRequestInterpreter`'s `callClaude` seam stubbed (no live LLM): using the identical real `HABIT`/`FOOD_LOGGING`/`WEAKENING` fixture G-2/RGEF/CSSC-001 already proved, the Terminal Decision `kind` changes deterministically from `INITIATIVE` (no request present, matching the pre-existing baseline exactly) to `SILENCE` (an active, resolved `NUTRITION`/`FOOD_LOGGING` suppression control present) — with the underlying Opportunity, Contextual Meaning, Evidence Tier, Eligibility, RGEF feedback history, Trust, and Relationship Maturity all proven byte-identical between the two runs, the first divergence occurring exactly at Stage 6 Candidate formation; elapsed time alone (tested well beyond RGEF's own 14-day/7-day windows) never revokes an active request; editing/deleting the source record restores baseline behavior on the very next Decision Pass. New test files `tests/explicitRequestInterpreter.test.js` (44 tests) and `tests/eur001ProductionBackedAcceptance.test.js` (9 tests); `tests/memoryLayer.test.js` (+16) and `tests/initiativeEngine.test.js` (+13) extended; **full repository regression: 2140/2140 passing, 0 failing** (2058 pre-EUR-001 baseline, net +82). `APP_VERSION`/service-worker `VERSION` advanced from `2.43.0`/`v2.43.0` to `2.44.0`/`v2.44.0` — unlike CSSC-001, this Work Item genuinely changes an existing, live Terminal Decision's outcome (proven above) when a user explicitly, literally requests it, a new user-visible Coach behavior. Open/deferred, explicitly not resolved by this closure: any activity-level (Running/Swimming/etc.) or time-of-day vocabulary; any control intent other than `SUPPRESS_ORDINARY_INITIATIVE`; a historical request/reversal ledger (a later, separate positive statement never silently revokes an older still-active negative one — documented V1 limitation, not a defect); Recommendation Engine/Eligibility/Contextual Meaning/Safety as consumers (none reached by this Work Item); Conversation/Voice producers (architecturally compatible via the same producer-neutral raw-statement contract, not built); Trust, Relationship Maturity (both untouched, Affirmative Trust V1 remains paused). Approved by Head of Product + AI Architect and closed — see `docs/specs/EUR_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 LCSC-001 — Legacy Coach Safety Containment — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/LCSC_001_SPEC_v1.0.md`; a narrow, interim containment Work Item — explicitly not Safety Foundation design, not a Health/Safety Profile, not an Action Model, not a new Safety Layer, and not a reopening of `docs/specs/SL-001_SPEC_v1.0.md`, which remains closed and untouched. Closes two confirmed-live current exposure paths a dedicated investigation this session identified in the legacy, pre-Decision-System Coach message pipeline. **Change A:** the real, data-driven Adaptive-TDEE red-flag condition (rapid weight-loss rate + shrinking arm measurement) no longer reaches the legacy generative Coach path from any entry point (`js/trigger/triggerDomain.js`'s `evalRedFlag()` now returns `live:false`; a new deterministic `'redflag'` case was added to `triggerLocalText()`, since none existed before; `js/trigger/triggerController.js`'s `triggerLiveText()` gained an unconditional early return for this type) — per Head of Product direction, the deterministic replacement text prescribes no calorie/nutrition/workout adjustment, only that the observed trend warrants a pause and review. **Change B:** `js/coach/coachPromptComposer.js`'s `buildBasePrompt()` — confirmed the single function every remaining legacy generative caller's system prompt passes through — gains one new, unconditional, eight-concept bounded Safety-scope instruction (no diagnosis; no unsupported medical inference; no medical treatment instructions; no invented recovery timeline; respects an explicit user-reported medical restriction within its literal scope; conservative under materially uncertain Safety context; no unsupported workout prescription; no unsupported nutrition/recovery prescription from Safety-sensitive context) — **explicitly defense-in-depth prompt containment only, never represented as Safety classification, medical reasoning authority, deterministic validation, or a Stage-8/Stage-9 replacement**; USM-001's `userStated` fragment, the legacy `coachMemoryFragment()`, and B5's derived fragment are all unmodified and unaffected. **Change C:** two independent deterministic surfaces (`coachLine()`'s `protein` case; `triggerDomain.js`'s `proteinFoodHint()`, also consumed by `triggerLocalText()`'s `'low-protein'` case) no longer name a specific food (egg/chicken/cottage cheese), since no authoritative dietary/allergy/restriction context exists — protein-gap awareness and encouragement preserved; `proteinFoodHint()`'s own export/facade (`js/app.js:1756`) unchanged, only its internal behavior. **Legacy Coach path status:** now explicitly recorded as **transitional architecture** — may continue operating, receives containment/maintenance only, no new Product capability; long-term direction is replacement by the canonical Coach Decision System as canonical capabilities become available. This closure does **not** establish canonical Safety architecture, a Stage-8/9 substitute, or evidence that further legacy-path Safety work is unnecessary — SL-001's own Health/Safety Profile gap remains exactly as open as SL-001's own closure left it. New test coverage extending `tests/triggerController.test.js`/`tests/triggerDomain.test.js`/`tests/coachPromptComposer.test.js`; **full repository regression: 2146/2146 passing, 0 failing** (2142 pre-LCSC-001 baseline, net +4). `APP_VERSION`/service-worker `VERSION` advanced from `2.44.0`/`v2.44.0` to `2.45.0`/`v2.45.0` — existing, already-live user-facing message content genuinely changed. Approved by Head of Product + AI Architect and closed — see `docs/specs/LCSC_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 FITME Safety Foundation Canonical Design approved and Canonical (`docs/governance/FITME_Safety_Foundation_Canonical_Design_v1.0.md`; freezes SF-DECOMP-01 — the Foundation A/User Safety Context, B/Minimum Action Identity, C/Canonical Safety Rule + Real Matcher decomposition, A and B independent, C dependent on both; documentation-only, no production code changes)
- 🟢 USC-001 — User Safety Context V1 (Safety Foundation A) — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/USC_001_SPEC_v1.0.md`; new `js/coachDecisionSystem/safetyContextInterpreter.js`, classifying explicit/literal user-reported Safety restrictions from Typed Memory only; `memoryLayer.js` extended to populate `pipelineContext.userSafetyContext`/`pipelineContext.availability.userSafetyContext`; PD-USC-01 temporal-qualifier preservation resolved; `index.html`/`sw.js`/17-file wiring-test update; new test file `tests/safetyContextInterpreter.test.js`, `tests/memoryLayer.test.js` extended. Approved by Head of Product + AI Architect and closed — see `docs/specs/USC_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 MAI-001 — Minimum Action Identity V1 (Safety Foundation B) — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/MAI_001_SPEC_v1.0.md`; new, standalone `js/domain/activityIdentityVocabulary.js` — a closed six-value activity vocabulary (RUNNING/WALKING/CYCLING/SWIMMING/STRENGTH_TRAINING/PADEL), independent of Safety (AD-SF-03) and of `domainTopicVocabulary.js` (AD-SF-04); an optional, additive `Candidate.actionIdentity` field, confirmed absent/`null` for every existing Candidate including the live FOOD_LOGGING Candidate; AD-MAI-01 Stage 9 propagation boundary recorded; no live Candidate producer at this baseline, an explicit, disclosed scope boundary, not a defect — accordingly no `index.html`/`sw.js`/wiring-test change; new test files `tests/actionIdentityContract.test.js`, `tests/activityIdentityVocabulary.test.js`. Approved by Head of Product + AI Architect and closed — see `docs/specs/MAI_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 USP-001 — User Safety Provenance V1 (Safety Foundation prerequisite, downstream of A, upstream of C) — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/USP_001_SPEC_v1.0.md`; new `js/coachDecisionSystem/userSafetyProvenanceInterpreter.js`, classifying the literal source a user attributes to a stated restriction from Typed Memory only, no title-recognition or proper-name-to-role conversion (PD-USP-02); `memoryLayer.js` extended to populate `pipelineContext.userSafetyProvenance`; wired into the real, executing `assembleContext()` per the same convention USC-001 followed; `index.html`/`sw.js`/17-file wiring-test update; new test file `tests/userSafetyProvenanceInterpreter.test.js`, `tests/memoryLayer.test.js` extended. Approved by Head of Product + AI Architect and closed — see `docs/specs/USP_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 CSR-001 — Canonical Safety Rule V1 + Real Matcher (Safety Foundation C) — IMPLEMENTED, VERIFIED AND CLOSED (`docs/specs/CSR_001_SPEC_v1.0.md`; `js/coachDecisionSystem/safetyLayer.js`'s `matchCanonicalSafetyRules()` — previously SL-001's own honest, disclosed empty stub — now implements the repository's first real Canonical Safety Rule, `matchRunningMedicalRestrictionRule`: an explicit user-reported medical restriction against RUNNING (USC-001's `userSafetyContext`) joined via `sourceMemoryId` to a matching medical-source attribution (USP-001's `userSafetyProvenance`), matched against a Candidate whose `actionIdentity.activity === 'RUNNING'` (MAI-001's identity), disqualifying it at Stage 8; PD-FC-07 (closed Hebrew accepted-form set) and PD-FC-08 (alphanumeric token-boundary correction) resolved; Stage 8/9 signatures unchanged; SL-001 remains closed and untouched. New test file `tests/canonicalSafetyRule.test.js` (30 tests), `tests/safetyLayer.test.js` extended (+10 `CSR1-*` tests); **full repository regression: 2330/2330 passing, 0 failing**, per the implementation commit's own report. **Reachability note:** the rule is implemented, executable, and integrated into the existing Stage 8/9 Safety evaluation path, proven end-to-end against synthetic Candidate/`pipelineContext` fixtures; no live Candidate producer in the current repository attaches `actionIdentity` to a real Candidate, so the rule does not yet act on any live, production-generated Candidate — a matter of upstream production reachability, not an incomplete or infrastructure-only implementation. This closes the Safety Foundation initiative's own first real vertical (Foundations A, B, and C all closed). `APP_VERSION`/service-worker `VERSION`: USC-001 and USP-001 each advanced it in lockstep with their own wiring (current value `2.47.0`/`v2.47.0`); MAI-001 and CSR-001 introduced no version change, consistent with their own disclosed zero-wiring/no-new-file scope. Approved by Head of Product + AI Architect and closed — see `docs/specs/CSR_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 TRR-001 — Training Readiness & Recovery V1 — IMPLEMENTED / VERIFIED / CLOSED (`docs/specs/TRR_001_SPEC_v1.0.md`; instantiates the Training Readiness & Recovery V1 Canonical Decision Package and the Context-Aware AI Reasoning Foundation for the first live vertical driven by the 8th Product Reason, `ADAPT_TO_CURRENT_STATE`. **New:** `js/domain/activityReferenceNormalizer.js` (deterministic activity-text → MAI-001-token mapper); `js/coachDecisionSystem/readinessStateInterpreter.js`, `activityPreferenceInterpreter.js`, `activityOppositionInterpreter.js` (three bounded interpreters); `js/coachDecisionSystem/trainingReadinessReasoningComponent.js` (CARF Ch.08/09 reasoning component). **Modified, additively only:** `contextualMeaningPolicy.js`/`eligibilityEvaluator.js` (`VALID_REASON_CATEGORIES` +1, independently-maintained copies both extended); `eligibilityEvaluator.js` (`BOUNDED_ENGAGEMENT_POLICY`, generalizing RGEF's own hardcoded admission condition, RGEF's entry preserved byte-identical); `initiativeEngine.js` (`SOURCE_REASON_MATURITY_OVERRIDES` +1 entry); `internalPipelineOrchestrator.js` (one new reasoning-invocation branch, gated on `validReasonCategory === 'ADAPT_TO_CURRENT_STATE'`, positioned at CARF's own canonical Stage 5→6 seam); `memoryLayer.js`; `safetyLayer.js` (two new Canonical Safety Rules — `matchWalkingMedicalRestrictionRule`, mirroring CSR-001's RUNNING rule with its own closed WALKING Hebrew/English accepted-form vocabulary, deliberately excluding הולך/הולכת per the same homograph rationale as CSR-001's own רצה exclusion; and `matchUnresolvedActivitySafetyCoverageRule`, the approved "Option D / refined Option A" generic Rule producing `DEFERRED` for a `PHYSICAL_ACTIVITY` Candidate outside RUNNING/WALKING's own coverage whose Safety relevance cannot be resolved non-relevant by either mechanism); `index.html`/`sw.js` wiring for every new file. `Candidate` gains `actionCategory`/`activityReference` (open literal text) alongside MAI-001's closed `actionIdentity`, computed exclusively, downstream, by the new normalizer — never supplied by the model (the reasoning component's own output schema has no `actionIdentity` field). `opportunityProvenance.sameNeedId` now populated for every Candidate, not only TRR's own. Two Product/Architecture precision-review rounds corrected, before implementation: a Capability #10 exhaustive-list bug (§19's ten-item V1 Action Envelope was silently narrowed to a closed seven-item list; corrected to preserve the model's authority to propose a further bounded action) and a §28 blanket-rule bug (collapsed to `hasAnyRestriction ⇒ DEFERRED`, the rule TDP itself withdrew; corrected to the approved "Option D / refined Option A" two-mechanism design). **Stage-9 production-reachability finding:** a separate, narrow, read-only Product/Architecture pre-commit verification found, and empirically proved via direct execution of the real, unmodified production pipeline, that §28's `DEFERRED` disposition had no live enforcement in the real Stage 8→9 dispatch — an inherited, pre-existing limitation (Stage 9's `finalReview()` had never received Candidate identity, per MAI-001's own AD-MAI-01), first exposed, not introduced or worsened, by this Work Item, since TRR-001 is the first Canonical Safety Rule whose intended disposition is not a Stage-8 absolute-override type. Classified a genuine implementation/canonical blocker per explicit instruction not to soften the finding; resolved by a separate, narrow canonical correction — see the entry immediately below. New test files `tests/readinessStateInterpreter.test.js`, `activityPreferenceInterpreter.test.js`, `activityOppositionInterpreter.test.js`, `activityReferenceNormalizer.test.js`, `trainingReadinessReasoningComponent.test.js`, `trrSafetyCoverage.test.js`; extensions to `tests/contextualMeaningPolicy.test.js`, `eligibilityEvaluator.test.js`, `initiativeEngine.test.js`, `internalPipelineOrchestrator.test.js`, `memoryLayer.test.js`. A persistent, repository-resident deterministic Hebrew accepted/rejected-form test suite for WALKING was added at reconciliation (`trrSafetyCoverage.test.js`), closing a disclosed test-coverage gap relative to CSR-001's own RUNNING Hebrew test matrix — this does **not** constitute or substitute for native-speaker linguistic review. **TRR-specific regression: 433/433 passing.** Full repository regression (after the Stage-9 correction below): **2478/2478 passing.** **CLOSED.** The native-speaker Hebrew-speaker linguistic verification of `WALKING_TEXT_ACCEPTED_FORMS` was completed and APPROVED by Product/Canonical review before this closure — no item remains open. Preference V1 remains paused, unmodified; no Legacy Coach integration; no provider-session memory; MAI-001's six-token activity vocabulary unchanged. Approved by Head of Product + AI Architect; implementation VERIFIED — see `docs/specs/TRR_001_SPEC_v1.0.md` for full details and evidence.)
- 🟢 Stage-9 Winning-Candidate Safety Input — Canonical Decision — IMPLEMENTED / VERIFIED AND CLOSED (`docs/governance/FITME_Stage9_Winning_Candidate_Safety_Input_Canonical_Decision_v1.0.md`; a narrow, single-issue architectural correction discovered during TRR-001's own read-only pre-commit verification, not a TRR-001-specific fix. **Root cause:** Stage 9's `finalReview()` had never, since TASK-006, received Candidate identity — an artifact of TASK-006's original design predating Candidate-level Safety identity fields entirely, observed and preserved (not repaired) by MAI-001's own AD-MAI-01, which explicitly reserved the question for "a later, explicit canonical decision"; this document is that decision. The mechanism was inherited and pre-existing (identical for CSR-001's RUNNING rule), but its *consequence* was not equivalent: RUNNING's own disposition happens to be a Stage-8 absolute-override type, incidentally protected there, while TRR-001's new `DEFERRED`-intended `INSUFFICIENT` disposition deliberately is not, leaving it with zero real enforcement anywhere in the live dispatch — proven empirically (not merely argued) by direct execution of the real, unmodified production pipeline before this correction. **Approved correction (Option A):** for a `SINGLE_WINNER` Terminal Decision only, `decisionFormation.js` forwards the actual winning Candidate to `safetyPort.finalReview()` as an additive, optional third argument (`finalReview(terminalDecision, pipelineContext, candidate)`, chosen over a candidate-first reordering specifically to preserve byte-for-byte backward compatibility for every pre-existing call); `safetyLayer.js`'s `finalReview()` forwards it to the same, unmodified `matchCanonicalSafetyRules()` Stage 8 already calls. `TIED_SET` is explicitly out of scope and unchanged — no tied member is forwarded as a stand-in winner. **Modified, additively only:** `js/coachDecisionSystem/decisionFormation.js`, `safetyIntegrationPort.js`, `safetyLayer.js` (production — no other file); `tests/decisionFormation.test.js`, `tests/trrSafetyCoverage.test.js`, `tests/fixtures/safetyIntegrationPortTestDouble.js` (tests). Stage 8, `ABSOLUTE_OVERRIDE_RISK_TYPES`, every Canonical Safety Rule, every Safety enum, and Stage 9's own disposition-mapping logic are all unchanged — this restores Stage 9's already-canonical authority; it does not relocate, redesign, or expand it. **Production-backed verified** using the real, unmodified `WinnerSelection.select()` → `DecisionFormation.form()` → real `SafetyLayer` chain (no test double, except a sanctioned `MODIFIED`-reachability proof, since no production Rule produces that disposition today): RUNNING + confirmed medical restriction → Stage 8 disqualification → `SILENCE`, unchanged; **CYCLING + unresolved restriction → Stage 8 `SINGLE_WINNER` → Stage 9 `DEFERRED` → `SILENCE`** (previously `INITIATIVE` — the exact defect this correction resolves, the critical evidence Product/Architecture accepted); Pilates + RUNNING-identified restriction → `UNMODIFIED` → ordinary deliverable decision; zero restrictions → `UNMODIFIED`; `MODIFIED` reachability confirmed live/compatible; `TIED_SET` confirmed unchanged even against a restriction that would `DEFER` a real `SINGLE_WINNER` candidate. New tests in `tests/decisionFormation.test.js` (+3) and `tests/trrSafetyCoverage.test.js` (+6 live-path acceptance tests, +4 persistent WALKING Hebrew accepted/rejected-form tests closing a disclosed test-coverage gap). **Full repository regression: 2478/2478 passing, 0 failing** (2465 pre-correction baseline including TRR-001's own implementation, net +13). No new Safety enum, disposition, `reasonCode`, or Rule; no `TIED_SET` semantic change; no MAI-001 vocabulary expansion. `APP_VERSION`/service-worker `VERSION` inspected at this closure and left unchanged (`2.47.0`/`v2.47.0`) — an internal Safety-dispatch correction with no new occurrence, timing, or content change to any already-approved Terminal Decision path other than correctly enforcing an already-approved disposition (`DEFERRED`) that was never, in fact, reachable before. Approved by Head of Product + AI Architect and closed — see `docs/governance/FITME_Stage9_Winning_Candidate_Safety_Input_Canonical_Decision_v1.0.md` for full details and evidence.)
- 🟢 DUC-001 — Direct User Coach Admission V1 — IMPLEMENTED / VERIFIED / CLOSED (`docs/specs/DUC_001_SPEC_v1.0.md`; the first Work Item admitting a real, direct user message into the governed Coach Decision System, instantiating `docs/governance/FITME_Direct_User_Coach_Admission_V1_Canonical_Decision_Package_v1.0.md`. `registerCoachDecisionSystem.js` gains a second trigger, `USER_MESSAGE_SUBMITTED`; `internalPipelineOrchestrator.js` gains a distinct `DIRECT_TURN_PASS` action path (`runDirectTurnPass()`), reusing Stage 4–10 (Evidence, Eligibility, Reasoning, Safety, Decision Formation, Expression) completely unmodified. **New:** `js/coachDecisionSystem/turnUnderstandingInterpreter.js` (bounded, domain-agnostic Turn Understanding — `interpretationStatus`/`affirmativeRequest`/`currentStateStatement`/`negativeControlPresent`/`desireOnlyPresent`); `js/coachDecisionSystem/conversationalNeedCreator.js` (the fifth Stage-3 contributor — domain-agnostic Need recognition, Step A, gated only on a classified affirmative request, never on domain/topic match; professional-capability resolution, Step B, a single deterministic equality check, `WORKOUT`/`WORKOUT_FREQUENCY` the one V1-live pair); `js/ui/coachConversationPresenter.js` (a shared, domain-agnostic Coach Conversation Surface — the same rendering slot for the TRR-supported path, clarification, and `UNSUPPORTED` content alike, never a direct `callClaude`/`ClaudeProxyClient`/`CoachClient` path). **New `TerminalDecision.kind = 'UNSUPPORTED'`** (Product/Architecture-authorized, decoupled from Safety's own disposition machinery) for a recognized-but-unsupported direct request — `decisionFormation.js` gains `formUnsupportedCapabilityOutcome()`; `expressionInputGate.js`/`deliveryIntentContract.js` gain the closed-enum addition; `expressionRenderer.js` gains a fifth, dedicated rendering path; the four existing `RECOMMENDATION`/`INITIATIVE`/`BOUNDARY`(`REFUSAL`/`ESCALATION`) paths and `BOUNDARY`'s own Safety-disposition coupling are fully unchanged. **Modified, additively only:** `eligibilityEvaluator.js` (`BOUNDED_ENGAGEMENT_POLICY` gains `DIRECT_USER_REQUEST`, servicing every relationship stage from Observer, since the user themselves initiated the interaction); `memoryLayer.js` (`assembleContext()` gains an additive second parameter — the Current User Turn — feeding `readinessStateContext` with a new `CURRENT_TURN` provenance value and `explicitRequestControls`, making a negative-control clause in live current-turn text reachable by EUR-001's own existing, completely unmodified suppression check for the first time); `evidenceEvaluator.js` (`DIRECT_USER_REQUEST → EXPLICIT_USER_STATEMENT`, the highest existing evidence-sufficiency tier — the user's own direct, current statement, by definition, requiring no inferred pattern); `app.js`/`index.html`/`sw.js` (`runUserMessageEngine()`, the Coach Conversation Surface markup and nav entry, script/precache registration). **First supported professional conversational capability: TRR** (Training Readiness & Recovery), reached through the existing, unmodified `ADAPT_TO_CURRENT_STATE` reasoning gate — proven end-to-end for the canonical dogfood turn `"ישנתי 5 שעות, כדאי לי להתאמן היום?"` with **zero Habit signal configured**, unlike the proactive path. Clarification is handled by the existing `CLARIFICATION_NEEDED` reasoning outcome plus a new, opaque, client-retained `clarificationRef`/`clarificationContext` round-trip — a fresh `turnId` and a full, ungated Decision Pass on the answering turn, never a resumed server-side conversation (no provider-session memory introduced). **Turn-Serving Post-Implementation Correction** (found by a dedicated Product/Architecture review after initial implementation, empirically proven by direct execution of the real, unmodified pipeline): during a `DIRECT_TURN_PASS`, an unrelated, real, higher-canonical-tier proactive Candidate (`CONFIRMED_PATTERN_ANTICIPATION`, tier 4) could displace the Candidate answering the originating turn (`DIRECT_USER_REQUEST`, then a global tier 5) — the existing, unmodified `hierarchyTier`-ascending Prioritization rule doing exactly what it always does, applied to two Opportunities that should never have competed in the same turn-serving pass. **Root principle established:** Decision-Pass causality/turn-serving scope is not the same concept as Canonical Candidate hierarchy. **Approved correction:** `internalPipelineOrchestrator.js` gains `isAdmittedForTurnServingPass()` and `buildOpportunitiesForDecisionPass()`'s own additive, optional third parameter (`currentTurnId`, supplied only by `runDirectTurnPass()`) — an Opportunity is admitted to a `DIRECT_TURN_PASS` iff `safetyHighRiskBypass === true` (Safety, unconditional, never required to carry the originating `turnId`) or its own `turnId` matches the originating turn — domain-agnostic by construction, never `sourceCategory`/domain/topic/opaque `needRef`-or-`sameNeedId` parsing; `APP_READY`'s own single-argument call site is untouched. The global, source-only `DIRECT_USER_REQUEST` hierarchy-tier/category mapping (`recommendationCategories.js`) was removed — `DIRECT_USER_REQUEST` describes *why* FITME is responding now, never a universal professional-capability tier — replaced by a narrow, `initiativeEngine.js`-owned `SOURCE_REASON_HIERARCHY_TIER_OVERRIDES` table, mirroring the existing `SOURCE_REASON_MATURITY_OVERRIDES` precedent: only `DIRECT_USER_REQUEST × ADAPT_TO_CURRENT_STATE → 5` is authorized; every pre-DUC source-only mapping is byte-unchanged. The dead, unused `SOURCE_CATEGORY_MAP.DIRECT_USER_REQUEST` entry was removed (`DIRECT_USER_REQUEST` remains a valid Opportunity Source). `Candidate.confidence: 1`'s own semantic (request-existence confidence, never professional-advice confidence, the latter remaining `rationale.uncertainty`'s own exclusive concern) was investigated and confirmed consistent with the architecture-wide meaning `Candidate.confidence` already carried — comment-only clarification, no runtime change. The Coach Conversation Surface's error color was corrected from a hardcoded hex value to the existing `var(--color-danger)` design token. **Production-backed verified:** a real unrelated `CONFIRMED_PATTERN_ANTICIPATION` Candidate no longer displaces the turn's own `DIRECT_USER_REQUEST` Candidate — `candidatePoolSize: 1`, winner `sourceCategory: DIRECT_USER_REQUEST`, re-confirmed against the committed repository state. New test files `tests/turnUnderstandingInterpreter.test.js` (31), `tests/conversationalNeedCreator.test.js` (14), `tests/duc001ProductionBackedAcceptance.test.js` (15, including the collision regression and the Safety-exemption test); extensions to `tests/coachDecisionSystemWiring.test.js`, `tests/c1Wp10Wiring.test.js`, `tests/compositionInlineStyleAudit.test.js`, `tests/deliveryIntentContract.test.js`, `tests/eligibilityEvaluator.test.js`, `tests/recommendationCategories.test.js`, `tests/initiativeEngine.test.js`, `tests/internalPipelineOrchestrator.test.js`. **Full repository regression: 2559/2559 passing, 0 failing** (2486 pre-DUC-001 baseline, net +73). No new Safety enum, disposition, or Rule; no EUR-001/CARF/RGEF/SL-001 redesign; Safety remains fully authoritative and unconditional; `APP_READY` and the proactive TRR path are unchanged; no raw Claude → DOM path exists anywhere in this contract; no automatic Memory/Firestore persistence of raw turn text; Preference V1 remains paused, unmodified. `APP_VERSION`/service-worker `VERSION` inspected at this closure and left unchanged (`2.47.0`/`v2.47.0`), consistent with TRR-001's own most-recent no-bump precedent. Approved by Head of Product + AI Architect; implementation VERIFIED — commit `4d4e4bf48b75ca09b7d8b31ba9c56d5c09d6dcff` — see `docs/specs/DUC_001_SPEC_v1.0.md` for full details and evidence.)
