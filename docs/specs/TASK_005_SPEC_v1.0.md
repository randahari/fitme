...
- **Explicit user statements or actions as an Opportunity source.** D1 Unit 05 lists this as a canonical source category but does not assign its Stage-3 detection to any specific engine (unlike the other four sources); D2 Unit 07 does not name the Initiative Engine as a contributor for it either. Recorded as **Repository Gap** (Section 36, item G-2) — no canonical source assigns ownership of this category's Stage-3 detection, so this document does not assign it to the Initiative Engine.
...
- **Exclusions:** D1-OD-01: absent safety/high-risk or explicit-statement/action exemptions, "a single event SHALL be treated as data, not evidence."
...
- **Exclusions:** none.
- **Ownership:** D1 Unit 05 lists this as a canonical source category but **no canonical source assigns its Stage-3 detection to any specific engine** — D2 Unit 07 does not name the Initiative Engine (or any other engine) as its contributor. Recorded as **Repository Gap** G-2 (Section 36); this document does not assign this source's detection to the Initiative Engine.
- **Output/pass condition:** N/A pending the gap above.
...
InitiativeCandidate {
  ...
  action: <proposed content>,               // required — the substance of the Initiative, not yet expressed as user-facing language (D1-CDO-03)
...
### G-2 — Repository Gap — **Status: Follow-up (Canonical Closure Decision)**: Stage-3 ownership of "Explicit User Statements or Actions"
D1 Unit 05 lists this as a canonical Opportunity source; no canonical source (D1 or D2) assigns its Stage-3 detection to any specific engine. **Blocks READY:** No — reclassified as a Follow-up by the Canonical Closure Decision; this document does not assign it to the Initiative Engine and treats it as out of TASK-005's functional scope. **Tracked as future work:** Architecture (AI Architect) to assign ownership, or confirm it is intentionally unowned/deferred.
...
### A-2 — Architecture Decision Pending — **Status: Follow-up (Canonical Closure Decision)**: Extending C2's suppression mechanism to an Initiative surface
C2 explicitly scopes `evaluateSuppression()` to the Trigger and Adaptive TDEE surfaces only ("No Recommendation/Initiative/Decision Engine... not built as part of C2"). Section 21's "rejection/suppression feedback from C2 where relevant" cannot be implemented without this extension. **Blocks READY:** No — reclassified as a Follow-up by the Canonical Closure Decision. **Tracked as future work:** AI Architect, if D1-IP-08 (no repeating an ignored Initiative) is to be enforced via the same structured mechanism the Recommendation Engine already uses, rather than a wholly separate one.
...
| No repeating ignored Initiative (D1-IP-08) | D1-IP-08 | (new) `initiativeEngine.js`; C2 extension is a Follow-up (A-2), not a READY precondition | 34.1 | 33.8 | pending implementation; A-2 tracked as Follow-up |
